...
public class CircularBuffer<T> implements UnsafeBlockingQueue<T>
{
  ...
  public final static int MAX_SIZE = 1 << 30;
  ...
  {
    if (n < 0) throw new IllegalArgumentException(String.format("Error: value cannot be negative: %d", n));
    if (n > MAX_SIZE) throw new IllegalArgumentException(String.format("Error: value too large: %d", n));
